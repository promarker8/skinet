...
export class ShopService {
  ...
  getProducts(useCache = true): Observable<Pagination<Product[]>> {
    ...
    if (this.productCache.size > 0 && useCache) {
      if (this.productCache.has(Object.values(this.shopParams).join('-'))) {
        // pagination is going to store the response --> product items, total count, items per page, page no.
        this.pagination = this.productCache.get(Object.values(this.shopParams).join('-'));
        if (this.pagination) return of(this.pagination);
      }
    }

    // set the fields that were intialised empty in the component?

    let params = new HttpParams();

    if (this.shopParams.brandId) params = params.append('brandId', this.shopParams.brandId);
  ...
  getBrands() {
    // check if we have this in our service first
    if (this.brands.length > 0) return of(this.brands);

    // before going out to the API to get this info (and saving it then, in the list in the service)
